...
z_3 = a_2*Theta2';
h_x = sigmoid(z_3);
delta_3 = zeros(size(h_x));
for i=1:num_labels
    y_k = y==i;
    J = J-(1/m)*(log(h_x(:,i))'*y_k+log(1-h_x(:,i))'*(1-y_k));
    delta_3(:,i) = (h_x(:,i)-y_k);
end
...
Theta1_grad(:,2:end) = Theta1_grad(:,2:end)*(1/m)+(lambda/m)*Theta1(:,2:end);
Theta2_grad(:,2:end) = Theta2_grad(:,2:end)*(1/m)+(lambda/m)*Theta2(:,2:end);
